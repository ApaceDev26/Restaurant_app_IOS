...
import 'package:toto_partner/common/widgets/discount_tag_widget.dart';
import 'package:toto_partner/common/widgets/not_available_widget.dart';
...
import 'package:toto_partner/features/restaurant/widgets/product_delete_bottom_sheet.dart';
  ...
  Widget build(BuildContext context) {
    ...
    if (product.variations != null && product.variations!.isNotEmpty) {
      for (int i = 0; i < product.variations!.length; i++) {
        for (int j = 0;
            j < product.variations![i].variationValues!.length;
            j++) {
          if (_stringToInt(
                  product.variations![i].variationValues![j].currentStock)! >
              0) {
            isOutOfStock = false;
          } else {
            isOutOfStock = true;
          ...
          children: [
            Container(
              padding: const EdgeInsets.symmetric(
                  vertical: Dimensions.paddingSizeSmall, horizontal: 13),
              ...
              child: Column(children: [
                ...
                InkWell(
                  ...
                  child: Container(
                    ...
                    child: Image.asset(Images.penIcon,
        ...
        child: InkWell(
          onTap: () => Get.toNamed(RouteHelper.getProductDetailsRoute(product),
              arguments: ProductDetailsScreen(
                  product: product, isCampaign: isCampaign)),
          child: Container(
            padding: const EdgeInsets.all(Dimensions.paddingSizeSmall),
            decoration: BoxDecoration(
            ...
            child: Row(children: [
              /*(product.imageFullUrl != null && product.imageFullUrl!.isNotEmpty) ? */ Stack(
                  children: [
                    ClipRRect(
                      borderRadius:
                                  ...
                                  children: [
                                    ...
                                    const SizedBox(
                                        width:
                                            Dimensions.paddingSizeExtraSmall),
                                    CustomToolTip(
                                      message: (product.stockType ==
                                                  'unlimited' ||
                                              product.itemStock! <= 0)
                                          ? 'your_main_stock_is_out_of_stock'.tr
                                          : 'one_or_more_variations_are_out_of_stock'
                                              .tr,
