...
import 'package:toto_partner/features/chat/domain/models/message_model.dart';
...
class ChatController extends GetxController implements GetxService {
  ...
  Future<void> _pollForNewMessages() async {
    ...
    if (_currentNotificationBody == null ||
    ...
    } else if (_currentNotificationBody!.customerId != null ||
        _currentNotificationBody!.type == UserType.customer.name ||
      ...
      response = await chatServiceInterface.getMessages(
        ...
        _currentNotificationBody!.type == UserType.delivery_man.name) {
      response = await chatServiceInterface.getMessages(
          1,
...
/*
  ...
  Future<void> getMessages(int offset, NotificationBodyModel? notificationBody,
    ...
    } else if (notificationBody.customerId != null ||
  ...
  Future<Response?> sendMessage(
    ...
    List<MultipartBody> chatImages = chatServiceInterface.processImages(
        _chatImage, objFile, _pickedVideoFile);

    if (notificationBody == null || notificationBody.adminId != null) {
      response = await chatServiceInterface.sendMessage(
          message, chatImages, 0, null, UserType.admin);
    } else if ((notificationBody.customerId != null ||
        notificationBody.type == UserType.customer.name)) {
      response = await chatServiceInterface.sendMessage(message, chatImages,
