...
class ChatRepository implements ChatRepositoryInterface {
  ...
  Future<ConversationsModel?> getConversationList(
    ...
    Response response = await apiClient.getData(
        '${AppConstants.getConversationListUri}?offset=$offset&limit=10&type=$type');
  ...
  Future<Response> getMessages(
    ...
    return await apiClient.getData(
        '${AppConstants.getMessageListUri}?offset=$offset&limit=10&${conversationID != null ? 'conversation_id' : userType == UserType.admin ? 'admin_id' : userType == UserType.delivery_man ? 'delivery_man_id' : 'user_id'}=${conversationID ?? userId}');
  }
