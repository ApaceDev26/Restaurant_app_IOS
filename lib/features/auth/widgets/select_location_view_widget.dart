...
import 'package:toto_partner/features/auth/widgets/location_search_dialog_widget.dart';
import 'package:toto_partner/features/auth/widgets/zone_selection_view_widget.dart';
...
class _SelectLocationViewWidgetState extends State<SelectLocationViewWidget> {
  ...
  GoogleMapController? get _activeMapController => widget.fromView
      ? _mapController
      : (_screenMapController ?? widget.mapController);
  ...
  void _applyZonePolygon(int? index) {
    if (index == null || index < 0) return;
    final zones = Get.find<LocationController>().zoneList;
    if (zones == null || index >= zones.length) return;
    final points = zones[index].formatedCoordinates;
            ...
            Row(children: [
              Expanded(
                child: ZoneSelectionWidget(
                  zoneList: zoneList,
                  mapController: _activeMapController,
                  onZoneSelected: _applyZonePolygon,
                ),
              ),
            ]),
              ...
              child: Stack(clipBehavior: Clip.none, children: [
                GestureDetector(
                  ...
                  child: GoogleMap(
                    ...
                    zoomControlsEnabled: !widget.fromView ? true : false,
                    ...
                    onMapCreated: (GoogleMapController controller) {
                      ...
                      final selected = locationController.selectedZoneIndex;
                      if (selected != null && selected >= 0) {
                        locationController.setZoneIndex(
                          selected,
                          mapController: controller,
                        );
                        _applyZonePolygon(selected);
                      } else {
                ...
                Center(
                    child:
                        Image.asset(Images.pickMarker, height: 30, width: 20)),
                widget.fromView
                    ? Positioned(
                        top: 10,
                        left: 10,
