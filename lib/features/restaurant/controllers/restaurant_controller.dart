...
import 'package:toto_partner/features/restaurant/domain/enum/time_type.dart';
import 'package:toto_partner/features/restaurant/domain/models/cuisine_model.dart';
...
import 'package:toto_partner/features/restaurant/domain/models/variant_type_model.dart';
import 'package:toto_partner/features/restaurant/domain/models/variation_model.dart';
import 'package:toto_partner/features/restaurant/domain/models/vat_tax_model.dart';
...
class RestaurantController extends GetxController implements GetxService {
  ...
  void initRestaurantData(Restaurant restaurant) {
    ...
    _isDineInEnabled =
        (Get.find<SplashController>().configModel?.dineInOrderOption ??
                false) &&
            (restaurant.isDineInActive == true);
    setTimeType(
  ...
  void setCategory(
    ...
    getProductList(
        ...
        foodType: foodType,
        stockType: stockType,
        categoryId: _categoryIndex != 0 ? _categoryIdList![index] : 0);
    update();
  }
  ...
  Future<void> getProductList(
    ...
    if (!_offsetList.contains(offset)) {
      ...
      ProductModel? productModel = await restaurantServiceInterface
          .getProductList(offset, foodType, stockType, categoryId);
  ...
  Future<void> updateRestaurantBasicInfo(
      Restaurant restaurant, List<Translation> translation) async {
    _isLoading = true;
    update();

    bool isSuccess = await restaurantServiceInterface.updateRestaurantBasicInfo(
        restaurant, _pickedLogo, _pickedCover, translation, _pickedMetaImage);
    if (isSuccess) {
      await Get.find<ProfileController>().getProfile();
      Get.back();
      showCustomSnackBar('restaurant_edit_updated_successfully'.tr,
          isError: false);
    }
    _isLoading = false;
    update();
  ...
  Future<void> updateRestaurant(
      Restaurant restaurant, List<String> cuisines) async {
    _isLoading = true;
    update();
    ...
    bool isSuccess = await restaurantServiceInterface.updateRestaurant(
        restaurant, cuisines, characteristics, tags);
    if (isSuccess) {
      await Get.find<ProfileController>().getProfile();
      Get.back();
  ...
  bool checkDeliveryWarning(bool delivery, bool takeAway) {
    if (!delivery && !takeAway) {
      showCustomSnackBar('can_not_disable_both_delivery_and_take_away'.tr,
          isError: true);
    }
  ...
  Future<void> _getNutritionSuggestionList() async {
    ...
    _selectedNutrition = [];
    List<String?>? suggestionList =
        await restaurantServiceInterface.getNutritionSuggestionList();
    if (suggestionList != null) {
  ...
  Future<void> _getAllergicIngredientsSuggestionList() async {
    ...
    List<String?>? suggestionList =
        await restaurantServiceInterface.getAllergicIngredientsSuggestionList();
    if (suggestionList != null) {
  ...
  Future<void> getVatTaxList() async {
    List<VatTaxModel>? vatTaxList =
        await restaurantServiceInterface.getVatTaxList();
    if (vatTaxList != null) {
      _vatTaxList = [];
