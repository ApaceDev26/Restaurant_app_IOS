...
import 'package:toto_partner/features/advertisement/widgets/create_advertisement_video_view_shimmer.dart';
import 'package:toto_partner/features/advertisement/widgets/custom_check_box.dart';
import 'package:toto_partner/features/advertisement/widgets/dotted_video_border.dart';
import 'package:toto_partner/features/advertisement/widgets/preview_profile_promotion_widget.dart';
import 'package:toto_partner/features/advertisement/widgets/preview_video_dialog_widget.dart';
...
class _CreateAdvertisementScreenState extends State<CreateAdvertisementScreen>
  ...
  void _editSetup() {
    ...
    validationController?.text =
        "${DateConverter.stringToMDY(widget.adsDetailsModel?.startDate ?? "")} - ${DateConverter.stringToMDY(widget.adsDetailsModel?.endDate ?? "")}";
    ...
    Get.find<AdvertisementController>().dateTimeRange = DateTimeRange(
      start: DateConverter.isoUtcStringToLocalDateOnly(
          widget.adsDetailsModel?.startDate ?? ""),
      end: DateConverter.isoUtcStringToLocalDateOnly(
              ...
              child: Column(
                  ...
                  children: [
                    ...
                    adsController.selectedAdsType ==
                            AdsType.restaurant_promotion.name
                        ? const SizedBox(height: Dimensions.paddingSizeLarge)
                        : const SizedBox(),
                    adsController.selectedAdsType ==
                            AdsType.restaurant_promotion.name
            ...
            child: Stack(
              ...
              children: [
                const CustomAssetImageWidget(
                    image: Images.previewImage, height: 100, width: 100),
                Positioned(
                  bottom: -7,
                  left: -3,
                  right: -3,
  ...
  String modifyDateRange(DateTimeRange? dateTimeRange) {
    String firstDate =
        DateConverter.stringToMDY(dateTimeRange?.start.toString() ?? "");
    String lastDate =
        DateConverter.stringToMDY(dateTimeRange?.end.toString() ?? "");
