...
class SubscriptionRepository implements SubscriptionRepositoryInterface {
  ...
  Future<Response> renewBusinessPlan(
    ...
    return await apiClient.postData(AppConstants.businessPlanUri, body,
        headers: headers, handleError: false);
  }
  ...
  Future<SubscriptionTransactionModel?> getSubscriptionTransactionList(
    ...
    if (response.statusCode == 200) {
      subscriptionTransactionModel =
          SubscriptionTransactionModel.fromJson(response.body);
    }
  ...
  Future<CheckProductLimitModel?> getProductLimit(
      {required int restaurantId, required int packageId}) async {
    CheckProductLimitModel? checkProductLimitModel;
    Response response = await apiClient.getData(
        '${AppConstants.checkProductLimitsUri}?restaurant_id=$restaurantId&package_id=$packageId');
    if (response.statusCode == 200) {
