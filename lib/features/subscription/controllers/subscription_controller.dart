...
import 'package:toto_partner/features/subscription/domain/models/check_product_limit_model.dart';
import 'package:toto_partner/features/subscription/widgets/renew_subscription_plan_bottom_sheet.dart';
import 'package:toto_partner/features/subscription/widgets/subscription_dialog_widget.dart';
import 'package:toto_partner/features/subscription/widgets/trial_end_bottomsheet.dart';
...
import 'package:toto_partner/features/subscription/domain/models/subscription_transaction_model.dart';
...
class SubscriptionController extends GetxController implements GetxService {
  ...
  Future<ResponseModel?> renewBusinessPlan(
      {required String restaurantId, required bool isCommission}) async {
    _renewLoading = true;
    update();
    ...
    Map<String, String> body = {
      ...
      'payment_method':
          _paymentIndex == 0 ? 'wallet' : _digitalPaymentName ?? '',
      'payment_gateway':
          _paymentIndex == 0 ? 'wallet' : _digitalPaymentName ?? '',
    ...
    if (response.statusCode == 200) {
      if (response.body['redirect_link'] != null) {
        ...
        Get.toNamed(RouteHelper.getPaymentRoute(
            digitalPaymentName, redirectUrl, null, false, null));
      } else {
        _renewStatus = 'packages';
        ...
        Get.back();
        showCustomSnackBar(
            response.body['restaurant_model'] == 'commission'
  ...
  void showDatePicker(BuildContext context) async {
    ...
    if (result != null) {
      ...
      update();
      getSubscriptionTransactionList(
          offset: '1', from: _from, to: _to, searchText: searchText);
    }
  }
      ...
      Future.delayed(const Duration(seconds: 1), () {
        showModalBottomSheet(
          ...
          builder: (con) => TrialEndBottomSheet(
              isTrial: Get.find<ProfileController>()
                      .profileModel!
                      .subscription
                      ?.isTrial ==
                  1),
  ...
  Future<void> getProductLimit(
      ...
      required Packages activePackage}) async {
    _isLoading = true;
    update();
