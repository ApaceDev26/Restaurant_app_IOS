...
class AddonController extends GetxController implements GetxService {
  ...
  Future<void> getAddonCategoryList() async {
    List<AddonCategoryModel>? addonCategoryList =
        await addonServiceInterface.getAddonCategory(
            moduleId: Get.find<ProfileController>()
                .profileModel!
                .restaurants![0]
                .id!);
  ...
  void setAddonCategory(int? id) {
    _addonCategoryId = id;
    _addonCategoryName = _addonCategoryList
        ?.firstWhere((category) => category.id == id,
            orElse: () => AddonCategoryModel(id: id, name: ''))
        .name;
  }
