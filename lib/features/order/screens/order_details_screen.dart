...
import 'package:toto_partner/common/widgets/custom_image_widget.dart';
...
import 'package:toto_partner/common/widgets/input_dialog_widget.dart';
...
import 'package:toto_partner/features/order/widgets/pdf_generate_dialog.dart';
import 'package:toto_partner/features/order/widgets/section_widget.dart';
...
import 'package:toto_partner/features/order/domain/models/order_details_model.dart';
import 'package:toto_partner/features/order/domain/models/order_model.dart';
import 'package:toto_partner/features/order/screens/invoice_print_screen.dart';
import 'package:toto_partner/features/order/widgets/available_deliveryman_bottom_sheet_widget.dart';
import 'package:toto_partner/features/order/widgets/camera_button_sheet_widget.dart';
import 'package:toto_partner/features/order/widgets/cancellation_dialogue_widget.dart';
import 'package:toto_partner/features/order/widgets/collect_money_delivery_sheet_widget.dart';
import 'package:toto_partner/features/order/widgets/dialogue_image_widget.dart';
import 'package:toto_partner/features/order/widgets/order_product_widget.dart';
import 'package:toto_partner/features/order/widgets/slider_button_widget.dart';
import 'package:toto_partner/features/order/widgets/verify_delivery_sheet_widget.dart';
import 'package:toto_partner/helper/date_converter_helper.dart';
import 'package:toto_partner/helper/notification_helper.dart';
import 'package:toto_partner/helper/price_converter_helper.dart';
import 'package:toto_partner/helper/responsive_helper.dart';
...
class _OrderDetailsScreenState extends State<OrderDetailsScreen>
  ...
  DateTime? _orderConfirmedAt;
  String? _previousOrderStatus;
  ...
  void _startApiCalling() {
    ...
    _timer = Timer.periodic(const Duration(seconds: 10), (timer) {
      Get.find<OrderController>()
          .setOrderDetails(OrderModel(id: widget.orderId));
    });
  }
  ...
  void initState() {
    ...
    Get.find<OrderController>()
      ...
      _tableNumberController.text =
          Get.find<OrderController>().orderModel?.orderReference?.tableNumber ??
              '';
      _tokenNumberController.text =
          Get.find<OrderController>().orderModel?.orderReference?.tokenNumber ??
              '';

      _ensureCancelTimerWhenConfirmedOrderLoaded();
    });
    ...
    Get.find<OrderController>().getOrderDetails(widget.orderId).then((_) {
      _ensureCancelTimerWhenConfirmedOrderLoaded();
    });
  ...
  void _startCancelButtonTimer() {
    _cancelButtonTimer?.cancel();
    _orderConfirmedAt = DateTime.now();
    _cancelButtonTimer = Timer(const Duration(seconds: 45), () {
      if (mounted) {
        setState(() {
          _orderConfirmedAt = null;
        });
      }
    });
  }

  /// Starts the short cancel window when opening details for an already-confirmed order.
  void _ensureCancelTimerWhenConfirmedOrderLoaded() {
    if (!mounted) return;

    final controllerOrderModel = Get.find<OrderController>().orderModel;

    if (controllerOrderModel != null &&
        controllerOrderModel.id == widget.orderId &&
        controllerOrderModel.orderStatus == 'confirmed' &&
        _orderConfirmedAt == null) {
      _startCancelButtonTimer();
    }
  }

  bool _shouldShowCancelButton(OrderModel? orderModel) {
    if (orderModel == null) return false;

    // For 'accepted' status, always show cancel button if other conditions are met
    if (orderModel.orderStatus == 'accepted') {
      return true;
    }

    // For 'confirmed' status, only show cancel button within 45 seconds
    if (orderModel.orderStatus == 'confirmed') {
      if (_orderConfirmedAt == null) {
        // Order was confirmed before we started tracking, don't show cancel button
        return false;
      }
      final elapsed = DateTime.now().difference(_orderConfirmedAt!);
      return elapsed.inSeconds <= 45;
    }
  ...
  Widget build(BuildContext context) {
    ...
    return GetBuilder<OrderController>(builder: (orderController) {
      ...
      final bool isStatusTransition = _previousOrderStatus != null &&
          _previousOrderStatus != controllerOrderModel?.orderStatus &&
          _previousOrderStatus != 'confirmed' &&
          _previousOrderStatus != 'accepted';
      final bool shouldStartConfirmCancelOnTransition =
      ...
      if (shouldStartConfirmCancelOnTransition) {
      ...
      bool showSlider = controllerOrderModel != null
          ? ((controllerOrderModel.orderStatus == 'pending') &&
                  ((controllerOrderModel.orderType == 'take_away' ||
                          restConfModel ||
                          selfDelivery) ||
                      controllerOrderModel.orderType == 'dine_in')) ||
              controllerOrderModel.orderStatus == 'confirmed' ||
      ...
      return PopScope(
        ...
        child: Scaffold(
          appBar: CustomAppBarWidget(
            ...
            subTitle: order != null && widget.orderModel.orderStatus != null
                ? '${'order_is'.tr} ${widget.orderModel.orderType == 'dine_in' && widget.orderModel.orderStatus == 'delivered' ? 'served'.tr : controllerOrderModel?.orderStatus?.tr ?? widget.orderModel.orderStatus!.tr}'
                : null,
            menuWidget: Row(
              children: [
                InkWell(
                ...
                SizedBox(
                    width: (!GetPlatform.isIOS && !GetPlatform.isWeb)
                            ...
                            child: Column(
                                ...
                                children: [
                                  ...
                                  SectionWidget(
                                    ...
                                    child: Column(children: [
                                      Row(children: [
                                        ...
                                        Text(
                                          DateConverter
                                              .dateTimeStringToDateTimeNewFormat(
                                                  order!.createdAt!),
                                          style: robotoRegular,
                                        ),
                                      ]),
                                  ...
                                  order.orderType == 'dine_in'
                                            ...
                                            Row(children: [
                                              Expanded(
                                                child: CustomTextFieldWidget(
                                                  ...
                                                  isEnabled: widget.orderModel
                                                              .orderStatus ==
                                                          'delivered'
                                                      ? false
                                                      : true,
                                                  hideEnableText: widget
                                                              .orderModel
                                                              .orderStatus ==
                                                          'delivered'
                                                      ? true
                                                      ...
                                                      child: CustomButtonWidget(
                                                        ...
                                                        onPressed: () {
                                                          ...
                                                          if (tableNumber
                                                              ...
                                                              10) {
                                                            showCustomSnackBar(
                                                                'table_number_cannot_exceed_10_characters'
                                                                    .tr);
                                                          } else if (tokenNumber
                                                                  .length >
                                                              10) {
                                                            showCustomSnackBar(
                                                                'token_number_cannot_exceed_10_characters'
                                                                    .tr);
                                                          } else {
                                                            orderController
                                                                .addDineInTableAndTokenNumber(
                                                              orderId: order.id,
                                                              tableNumber:
                                                                  tableNumber,
                                                              tokenNumber:
                                      ...
                                      : const SizedBox(),
                                  SizedBox(
                                      height: order.orderType == 'dine_in'
                                          ? Dimensions.paddingSizeSmall
                                          : 0),
                                  ...
                                  SectionWidget(
                                    title:
                                        '${'customer_details'.tr} ${order.isGuest! ? '(${'guest_user'.tr})' : ''}',
                                    child: Padding(
                                  ...
                                  (order.deliveryMan != null &&
                                      ...
                                      ? SectionWidget(
                                          ...
                                          titleWidget: canShowDeliveryMan
                                              ? InkWell(
                                                  onTap: () {
                                                          ...
                                                          orderController
                                                            ...
                                                            if (success) {
                                                              ...
                                                              await NotificationHelper
                                                                  .dismissForegroundNotificationForOrder(
                                                                      orderIdToDismiss);
                                                              Get.find<
                                                                      ProfileController>()
                                                                  .getProfile();
                                                  ...
                                                  label: Text(
                                                    (controllerOrderModel
                                                        ...
                                                        : (controllerOrderModel
                                                                ...
                                                                (controllerOrderModel.orderStatus ==
                                                                        'accepted' &&
                                                                    (controllerOrderModel.confirmed !=
                                                                            null ||
                                                                        controllerOrderModel.accepted !=
                                                                            null)))
                                                            ? 'swipe_to_cooking'
                                                                .tr
                                                            : (controllerOrderModel
