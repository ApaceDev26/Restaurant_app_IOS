...
class OrderRepository implements OrderRepositoryInterface {
  ...
  Future<PaginatedOrderModel?> getPaginatedOrderList(
    ...
    Response response = await apiClient.getData(
        '${AppConstants.completedOrdersUri}?status=$status&offset=$offset&limit=10&is_subscription=$isSubscription');
    if (response.statusCode == 200) {
  ...
  Future<bool> addDineInTableAndTokenNumber(
    ...
    Response response = await apiClient
        .postData('${AppConstants.addDineInTableAndTokenUri}/$orderId', {
      '_method': 'put',
